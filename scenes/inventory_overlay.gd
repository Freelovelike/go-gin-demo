	...
	for cid in keys:
		if inventory[cid] > 0:
			...
			if CROPS.size() > int(cid):
				...
				var nw = ThemeDB.fallback_font.get_string_size(nm, HORIZONTAL_ALIGNMENT_LEFT, -1, 16).x
				_draw_text(sx + 70 - nw * 0.5, sy + 70, nm, 16, Color(0.1, 0.1, 0.2))
				var at := "数量: " + str(inventory[cid])
				var aw = ThemeDB.fallback_font.get_string_size(at, HORIZONTAL_ALIGNMENT_LEFT, -1, 14).x
				...
				var pw2 = ThemeDB.fallback_font.get_string_size(pt, HORIZONTAL_ALIGNMENT_LEFT, -1, 10).x
